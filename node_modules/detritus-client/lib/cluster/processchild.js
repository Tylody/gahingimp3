...
class ClusterProcessChild extends detritus_utils_1.EventSpewer {
    constructor(cluster) {
        super();
        this._restRequestsWaiting = new basecollection_1.BaseCollection();
                ...
                case constants_1.ClusterIPCOpCodes.REST_REQUEST:
                    {
                        ...
                        if (clusterId === this.clusterId) {
                            const { error, hash, result } = message.data;
                            if (this._restRequestsWaiting.has(hash)) {
                                const waiting = this._restRequestsWaiting.get(hash);
                                if (error) {
                                    waiting.reject(new errors_1.ClusterIPCError(error));
                                }
                                else {
    ...
    async sendRestRequest(name, args) {
        // add a timeout
        ...
        const promise = new Promise(async (resolve, reject) => {
            if (this._restRequestsWaiting.has(hash)) {
                ...
                await this.sendIPC(constants_1.ClusterIPCOpCodes.REST_REQUEST, { args, hash, name }, true);
